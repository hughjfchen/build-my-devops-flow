...
defaultPlatformProject.pkgs.lib.mapAttrs (_: pkgs: rec {
  ...
  gh-webhook = import ./default.nix { nativePkgs = pkgs;
                                     customModules = [ { packages.gh-webhook.dontStrip = false; } ] ++ (if pkgs.stdenv.hostPlatform.isMusl then 
                                                        [
                                                          ...
                                                          { packages.gh-webhook.configureFlags = 
                                                            [
