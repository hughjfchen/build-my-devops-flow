{ pkgs, ... }: {
  config.project.name = "gh-webhook";
  config.services = {
    ...
    gh-webhook-service = {
      ...
      service.defaultExec = [
        "sh"
        "-c"
        "${pkgs.gh-webhook.java-analyzer-runner-exe.outPath}/bin/java-analyzer-runner --pageUrl https://en.swissquote.com/forex/pricing/trading-conditions --uploadUrl http://postgrest-service:3000/api/updateSQTradingCondition"
      ];
      service.ports = [ ];
      service.environment.ENV1 =
        "${pkgs.gh-webhook.java-analyzer-runner-exe.outPath}/share/doc";
