{ pkgs, ... }: {
  ...
  config.services = {
    ...
    exe-engine-service = {
      ...
      service.useHostStore = true;
      service.command = [
        "${pkgs.haskellPackages.exe-engine.outPath}/bin/postgrest"
        "${pkgs.exe-engine-config-local-file}"
      ];
      #[ "${pkgs.haskellPackages.exe-engine.outPath}/bin/postgrest" ];
      service.ports = [
    ...
    java-analyzer-uploader-service = {
      ...
      service.defaultExec = [
        "sh"
        "-c"
        "${pkgs.java-analyzer-uploader.java-analyzer-uploader-exe.outPath}/bin/java-analyzer-uploader --pageUrl https://en.swissquote.com/forex/pricing/trading-conditions --uploadUrl http://exe-engine-service:3000/api/updateSQTradingCondition"
      ];
      service.ports = [ ];
      ...
      service.stop_signal = "SIGINT";
      service.depends_on = [ "db-service" "exe-engine-service" ];
      service.links =
        [ "db-service:db-service" "exe-engine-service:postgrest-service" ];
    };
  };
}
